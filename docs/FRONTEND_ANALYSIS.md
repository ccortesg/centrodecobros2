...
- Addendum 2026-07-13: `Respuesta.vue` y `PagoRecibido.vue` agregan los criterios `Folio Operación` y `Núm. Autorización`. En `respuestas`, el numero de autorizacion se almacena fisicamente en `auth`; Pagos Recibidos normaliza ese valor con `pagospei.autorizacion` y `transaccionesDom.auth` sin modificar el esquema.
- Addendum 2026-07-13: la tabla de `Respuesta.vue` combina Referencia/Ref. Transacción y Date/Time; `NB Error` se consulta mediante icono, tooltip y modal. `Transaccion.vue` presenta `Vencido` con fondo rojo y texto negro. El folio `personas.num_documento` queda inmutable en la edicion de clientes.
- Addendum 2026-07-13: las acciones financieras directas activas ya exigen confirmacion antes de Axios: cancelacion de transaccion/domiciliacion, cargo recurrente manual y cancelacion de importacion. No se agregan dialogos redundantes a flujos que primero abren modal.
- Addendum 2026-07-13: `TransaccionDom.vue` expone filtros de texto por `transaccionesDom.foliocpagos` y `transaccionesDom.auth`. En Cargos Recurrentes y Respuestas, `approved`, `denied` y `error` mantienen su texto y reutilizan respectivamente los badges visuales de Activo, Vencido y Pendiente.
- Addendum 2026-07-14: en la tabla de `Respuesta.vue`, los selects de cantidad y status ocupan el primer renglón de sus encabezados y los títulos `Opciones`/`Status` el segundo. Todos los encabezados de esa tabla se centran horizontalmente y se alinean verticalmente abajo.
- Addendum 2026-07-14: `Respuesta.vue` divide también los encabezados Referencia/Ref. Transacción, Folio Operación y Date/Time en dos renglones, abrevia `CD Response` como `CD Resp.` y presenta `approved`/`denied` como `Aprobado`/`Denegado` sin cambiar los valores del filtro. Como `respuestas.date` es texto externo, la celda conserva el valor original cuando el formateador no reconoce su formato.
- Addendum 2026-07-14: las cuatro instancias de `Respuesta.vue` reciben `idrol`; para Cliente se oculta visualmente `NB Company`, mientras Administrador conserva la columna. El modal de detalle usa dos columnas compactas en escritorio y una columna responsive en móvil, sin modificar campos ni comportamiento backend.
